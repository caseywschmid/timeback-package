## OneRoster — Gradebook - Get Results for Class
...
- `TimebackGetResultsForClassRequest` with required fields:
...
# Example 1: Get results for a class with default parameters
request = TimebackGetResultsForClassRequest(
    class_sourced_id=class_sourced_id
)
resp = client.oneroster.gradebook.get_results_for_class(request)
...
request_filtered = TimebackGetResultsForClassRequest(
    class_sourced_id=class_sourced_id,
    query_params=query_params
)
resp_filtered = client.oneroster.gradebook.get_results_for_class(request_filtered)
...
- This endpoint returns all results associated with the specified class.
