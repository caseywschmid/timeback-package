## OneRoster — Gradebook - Get Results for Student for Class
...
- `studentSourcedId` (string, required): The sourcedId of the student
...
- `TimebackGetResultsForStudentForClassRequest` with required fields:
  - `class_sourced_id` (string): The sourcedId of the class (path parameter)
  - `student_sourced_id` (string): The sourcedId of the student (path parameter)
...
- 404: Class or student not found → raises `NotFoundError`
...
request_filtered = TimebackGetResultsForStudentForClassRequest(
    class_sourced_id=class_sourced_id,
    student_sourced_id=student_sourced_id,
    query_params=query_params
)
resp_filtered = client.oneroster.gradebook.get_results_for_student_for_class(request_filtered)
...
- This endpoint returns only results associated with the specified class and student.
...
- If the class or student does not exist, the API will return a 404 error with message 'Class or student not found.'
