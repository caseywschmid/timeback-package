"""Get Academic Session endpoint for OneRoster Rostering.

GET /ims/oneroster/rostering/v1p2/academicSessions/{sourcedId}
"""

from typing import Any, Dict
from timeback.http import HttpClient
from timeback.models.request import TimebackGetAcademicSessionRequest
from timeback.models.response import TimebackGetAcademicSessionResponse
from timeback.logs import logger

log = logger.configure_logging(__name__, log_level="DEBUG")


def get_academic_session(
    http: HttpClient, request: TimebackGetAcademicSessionRequest
) -> TimebackGetAcademicSessionResponse:
    """Fetch a single academic session by sourcedId.

    GET /ims/oneroster/rostering/v1p2/academicSessions/{sourcedId}
    """
    params: Dict[str, Any] = {}
    if request.query_params:
        params = request.query_params.to_query_dict()

    log.debug(f"Sourced ID: {request.sourced_id}")

    data = http.get(
        f"/ims/oneroster/rostering/v1p2/academicSessions/{request.sourced_id}", params=params
    )

    log.debug(f"Raw Data: {data}")
    return TimebackGetAcademicSessionResponse.model_validate(data)

