...
def test_get_classes_for_user_success():
    """Test successful retrieval of classes for a user."""
    mock_http = MockHttp()
    request = TimebackGetClassesForUserRequest(user_sourced_id="user-001")
    resp = get_classes_for_user(mock_http, request)

    assert isinstance(resp, TimebackGetAllClassesResponse)
    assert len(resp.classes) == 1
    assert "/ims/oneroster/rostering/v1p2/users/user-001/classes" in mock_http.last_path
