...
def test_get_classes_for_teacher_success():
    mock_http = MockHttp()
    request = TimebackGetClassesForTeacherRequest(teacher_sourced_id="teacher-001")
    resp = get_classes_for_teacher(mock_http, request)
    assert isinstance(resp, TimebackGetAllClassesResponse)
    assert "/ims/oneroster/rostering/v1p2/teachers/teacher-001/classes" in mock_http.last_path
