...
def test_get_results_for_class_success():
    """Test successful retrieval of results for a class."""
    request = TimebackGetResultsForClassRequest(
        class_sourced_id="class-789"
    )
    resp = get_results_for_class(MockHttp(), request)
    ...
    assert resp.results[1].sourcedId == "result-2"
    assert resp.results[1].scoreStatus == TimebackScoreStatus.SUBMITTED
    assert resp.total_count == 2
    assert resp.limit == 100


def test_get_results_for_class_with_query_params():
